...
export const BaseChat = React.forwardRef<HTMLDivElement, BaseChatProps>(
  (
    ...
    useEffect(() => {
      ...
      const shouldOpenAppsFromQuery = searchParams.get(APPS_VIEW_QUERY_KEY) === APPS_VIEW_QUERY_VALUE;
      const shouldOpenAppsFromStorage = localStorage.getItem(APPS_RESTORE_STORAGE_KEY) === APPS_VIEW_QUERY_VALUE;

      if (!shouldOpenAppsFromQuery && !shouldOpenAppsFromStorage) {
        return;
      }

      setActiveMainView('apps');
      localStorage.removeItem(APPS_RESTORE_STORAGE_KEY);

      if (shouldOpenAppsFromQuery) {
        searchParams.delete(APPS_VIEW_QUERY_KEY);
        ...
        <ClientOnly>
          {() => (
            <Menu
              ...
              onAppsClick={() => setActiveMainView((current) => (current === 'apps' ? 'chat' : 'apps'))}
            />
          )}
        </ClientOnly>
        <div className="flex flex-col lg:flex-row overflow-y-auto w-full h-full">
          {activeMainView === 'apps' ? (
            <div className="flex h-full min-w-0 flex-1 pl-0 lg:pl-[340px]">
              <AppsDashboard
                userId={composioUserId}
                onRequireIdentity={() => {
                  console.info('Preparing a local Apps identity.');
                }}
              />
            </div>
          ) : (
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    {progressAnnotations && (
                      <ProgressCompilation data={progressAnnotations} isStreaming={isStreaming} />
                    )}
                    <ChatBox
