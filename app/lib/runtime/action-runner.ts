...
const VITE_ENTRY_FILES = ['src/main.ts', 'src/main.tsx', 'src/main.js', 'src/main.jsx'];
const APP_COMPONENT_FILES = ['src/App.tsx', 'src/App.jsx', 'src/App.ts', 'src/App.js'];
const STYLE_ENTRY_FILES = ['src/index.css', 'src/styles.css'];

export interface ViteRepairPlan {
  isApplicable: boolean;
  shouldInstallDependencies: boolean;
  repairFiles: Array<{ path: string; content: string }>;
...
function getFirstExistingPath(existingPaths: Set<string>, candidates: string[]) {
  return candidates.find((candidate) => existingPaths.has(candidate));
}

function createDefaultIndexHtml(mainEntryFile: string) {
  return `<!doctype html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
  ...
  <body>
    <div id="root"></div>
    <script type="module" src="/${mainEntryFile}"></script>
  </body>
</html>
`;
}

function createDefaultMainEntry(appComponentFile: string, includeStyleImport: string | undefined) {
  const importPath = appComponentFile.startsWith('src/') ? `./${appComponentFile.slice(4)}` : `./${appComponentFile}`;
  const normalizedImportPath = importPath.replace(/\.(tsx|ts|jsx|js)$/, '');
  const styleImport = includeStyleImport ? `import './${includeStyleImport.slice(4)}';\n` : '';

  return `import React from 'react';
import ReactDOM from 'react-dom/client';
import App from '${normalizedImportPath}';
${styleImport}
...
export function getViteProjectRepairPlan(params: {
  packageJsonContent?: string;
  existingPaths: string[];
}): ViteRepairPlan {
  const existingPaths = new Set(params.existingPaths.map(normalizeProjectPath));
  const packageJson = parsePackageJson(params.packageJsonContent);
  const scripts = packageJson?.scripts ?? {};
  const dependencies = { ...(packageJson?.dependencies ?? {}), ...(packageJson?.devDependencies ?? {}) };
  const scriptValues = Object.values(scripts);

  const usesRemix =
    scriptValues.some((value) => value.includes('remix')) ||
    '@remix-run/react' in dependencies ||
    '@remix-run/dev' in dependencies;

  const usesViteScript = scriptValues.some((value) => /\bvite\b/.test(value));
  const hasViteDependency = 'vite' in dependencies;
  const hasViteConfig = VITE_CONFIG_FILES.some((file) => existingPaths.has(file));
  const isApplicable = !usesRemix && (usesViteScript || hasViteDependency || hasViteConfig);
  ...
  const existingMainEntry = getFirstExistingPath(existingPaths, VITE_ENTRY_FILES);
  const existingAppComponent = getFirstExistingPath(existingPaths, APP_COMPONENT_FILES);
  const existingStyleEntry = getFirstExistingPath(existingPaths, STYLE_ENTRY_FILES);
  const repairFiles: Array<{ path: string; content: string }> = [];
  const blockingIssues: string[] = [];

  let mainEntry = existingMainEntry;

  if (!mainEntry && existingAppComponent) {
    const defaultMainEntry = existingAppComponent.endsWith('.tsx') || existingAppComponent.endsWith('.ts') ? 'src/main.tsx' : 'src/main.jsx';

    repairFiles.push({
      path: defaultMainEntry,
      content: createDefaultMainEntry(existingAppComponent, existingStyleEntry),
    });
    mainEntry = defaultMainEntry;
  } else if (!mainEntry) {
    blockingIssues.push('Missing app entry file (`src/main.ts`, `src/main.tsx`, `src/main.js`, or `src/main.jsx`).');
  }
  ...
  if (!existingPaths.has('index.html')) {
    if (mainEntry) {
      repairFiles.push({
        path: 'index.html',
        content: createDefaultIndexHtml(mainEntry),
      });
    } else {
      blockingIssues.push('Missing `index.html` entry file.');
    }
  }

  if (!hasViteDependency && usesViteScript) {
    blockingIssues.push('`package.json` uses Vite scripts but does not declare a `vite` dependency.');
  }

  const hasInstalledVite = existingPaths.has('node_modules/.bin/vite') || existingPaths.has('node_modules/vite');

  return {
    isApplicable,
    shouldInstallDependencies: hasViteDependency && !hasInstalledVite,
    repairFiles,
    blockingIssues,
  };
}
...
export class ActionRunner {
  ...
  async #runStartAction(action: ActionState) {
    ...
    await this.#prepareProjectForStart(shell);
  ...
  async #prepareProjectForStart(shell: BoltShell) {
    const webcontainer = await this.#webcontainer;
    const candidatePaths = ['package.json', 'index.html', 'node_modules/.bin/vite', 'node_modules/vite', ...VITE_CONFIG_FILES, ...VITE_ENTRY_FILES, ...APP_COMPONENT_FILES, ...STYLE_ENTRY_FILES];
    ...
    for (const candidatePath of candidatePaths) {
      try {
        if (candidatePath === 'package.json') {
          packageJsonContent = await webcontainer.fs.readFile(candidatePath, 'utf-8');
        } else {
          await webcontainer.fs.readFile(candidatePath);
        }

        existingPaths.push(candidatePath);
      } catch {
        // Ignore missing files during preflight.
      }
    }

    const repairPlan = getViteProjectRepairPlan({
      packageJsonContent,
      existingPaths,
    });

    if (!repairPlan.isApplicable) {
      return;
    }

    for (const file of repairPlan.repairFiles) {
    ...
    if (repairPlan.shouldInstallDependencies) {
      const installResp = await shell.executeCommand(this.runnerId.get(), 'npm install --yes --no-audit --no-fund --silent', () => {
        throw new ActionCommandError('Dependency installation was aborted', 'The generated preview project could not finish installing its dependencies.');
      });

      if (installResp?.exitCode !== 0) {
        throw new ActionCommandError(
          'Failed To Install Preview Dependencies',
          installResp?.output || 'No output was captured while installing preview dependencies.',
        );
      }
    }

    if (repairPlan.blockingIssues.length > 0) {
      const repairSummary =
        repairPlan.repairFiles.length > 0
          ? `Applied automatic repairs:\n${repairPlan.repairFiles.map((file) => `- ${file.path}`).join('\n')}\n\n`
          : '';

      throw new ActionCommandError(
        'Generated preview project is incomplete',
        `${repairSummary}The generated app could not be previewed because it is missing required Vite files:\n${repairPlan.blockingIssues.map((issue) => `- ${issue}`).join('\n')}`,
      );
    }
  }
