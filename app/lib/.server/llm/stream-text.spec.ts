import { formatDataStreamPart, parseDataStreamPart, type ToolSet } from 'ai';
...
describe('streamText Composio and stream compatibility routing', () => {
  ...
  it('discards buffered native structural chunks before merge-time compatibility output', async () => {
  ...
  it('does not append compatibility output after native merge has written visible text', async () => {
    ...
    expect(writes).toEqual([nativeText]);
  });
